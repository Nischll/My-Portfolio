...
const Home = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        imageRef.current,
        { opacity: 0, x: 50 },
        { opacity: 1, x: 0, duration: 1.1, ease: "power3.out", delay: 0.1 }
      );

      // Content children stagger upward
      const children = gsap.utils.toArray<HTMLElement>(".hero-child", contentRef.current!);
      gsap.fromTo(
        children,
  ...
  return (
    ...
    >
      ...
      >
        <p className="hero-child text-sm sm:text-base font-medium tracking-widest text-[#A86B3E] uppercase mb-3">
          {hero.greeting}
        </p>
        <h1 className="hero-child text-5xl sm:text-6xl xl:text-7xl font-extrabold leading-[1.08] tracking-tight text-[#3B2F2F]">
          {hero.name}
        </h1>
        <p className="hero-child mt-4 text-lg sm:text-xl font-semibold text-[#5C4B45]">
          {hero.headline}
          <span className="mx-2 text-[#A86B3E]">·</span>
          {hero.subline}
        </p>
        <p className="hero-child mt-5 text-base sm:text-lg text-[#4C3D36] leading-relaxed max-w-lg">
          {hero.intro}
        </p>

        {/* CTA row */}
        <div className="hero-child mt-10 flex flex-wrap items-center gap-3">
          <a
            href="#projects"
        ...
        {/* Scroll hint */}
        <div className="hero-child mt-14 hidden lg:flex items-center gap-2 text-xs text-[#A86B3E]/60 font-medium tracking-wider uppercase">
