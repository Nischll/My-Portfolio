...
import {
  Code2, Database, Wrench, ChevronLeft, ChevronRight,
  Box, FileCode, Type, Layout, Palette, Sparkles,
  GitBranch, Layers, Globe, Server, Send,
} from "lucide-react";
...
function SkillCard({ group, isActive }: { group: (typeof skills.groups)[number]; isActive: boolean }) {
  const Icon = SKILL_ICONS[group.label];
  return (
    <div
      className={`flex flex-col rounded-2xl border bg-[#FAF7F4] p-5 sm:p-6 shadow-sm min-h-[260px] sm:min-h-[280px] transition-all duration-500 ease-out will-change-transform ${
        isActive
...
const SkillsSection = () => {
  ...
  const sectionRef = useScrollAnimation(".anim-child", 0.14, );
  ...
  return (
    ...
    >
      <div className="w-full">
        ...
        <p className="anim-child text-[#5C4B45] text-lg max-w-2xl mb-12">
          ...
          <div className="flex items-center justify-center gap-4 w-full max-w-5xl mx-auto">
            ...
            <div className="relative w-full max-w-4xl min-h-[420px] sm:min-h-[460px] flex items-center justify-center">
              {skills.groups.map((group, index) => {
                const position =
                  index === leftIndex ? "left" : index === current ? "center" : "right";
            ...
            {skills.groups.map((_, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all ${
                  i === current ? "w-6 bg-[#A86B3E]" : "w-2 bg-[#E8DDD4]"
                }`}
              />
            ))}
          </div>
